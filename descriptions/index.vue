...
.desc-box(:class="[border ? 'desc-border' : '']")
  .desc-item(
    ...
    .desc-default.desc-item-content(
      ...
      fragment(v-else) {{ item.render ? item.render(dataSource, dataSource ? dataSource[item.key] : '') : dataSource && dataSource[item.key] ? dataSource[item.key] : '- -' }}
</template>
<script>
/**
